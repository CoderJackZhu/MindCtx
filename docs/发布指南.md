...
| Obsidian | `mindctx` | `0.1.0` | Obsidian 社区插件 |
...
# 3. 提交并打 tag（Obsidian 要求 tag = manifest version，无任何前缀）
git add -A
...
git push origin main vscode-v0.2.0 0.2.0
```
...
| `.github/workflows/release-obsidian.yml` | tag `X.Y.Z`（纯版本号） | 构建并创建 GitHub Release |
...
- 确认 tag 格式正确：纯版本号（如 `0.2.0`），必须与 `manifest.json` 的 `version` 完全一致，**不能带 `v`/`obsidian-v` 前缀**（官方规则：Obsidian 按 manifest version 精确匹配同名 tag 的 release）
