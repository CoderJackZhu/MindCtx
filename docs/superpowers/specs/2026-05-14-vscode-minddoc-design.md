# VSCode MindDoc Extension Design Spec

## Overview

将 MindDoc Obsidian 插件的功能移植到 VSCode 平台，采用 Monorepo 结构共享核心逻辑，通过 Custom Editor API 提供与 Obsidian 版一致的双视图编辑体验。
...
│   │   │   │   └── json.ts
│   │   │   ├── bridge/
│   │   │   │   └── mindElixirBridge.ts  ← MindDocTree ↔ MindElixir 数据转换
│   │   │   ├── ai/
│   │   │   │   └── contextBuilder.ts    ← AI 上下文生成
│   │   │   └── utils/
│   │   │       └── debounce.ts
│   │   ├── package.json     ← name: @minddoc/core
│   │   └── tsconfig.json
...
│       ├── src/
│       │   ├── extension.ts             ← 扩展入口
│       │   ├── MindDocEditorProvider.ts  ← Custom Editor Provider
│       │   ├── MindDocDocument.ts       ← CustomDocument + edit tracking
...
使用 pnpm workspace。`@minddoc/core` 作为内部依赖被 obsidian 和 vscode 包引用。
...
**MindDocEditorProvider** (implements `CustomEditorProvider<MindDocDocument>`)

职责：
- 注册为 `.mind.md` 文件的默认编辑器（package.json 中 `customEditors` 配置 `priority: "default"`）
- 通过 `minddoc.openAs` 命令支持打开普通 `.md` 文件（实现：`vscode.commands.executeCommand('vscode.openWith', uri, 'minddoc.editor')`）
- 创建和管理 Webview 面板
...
**MindDocDocument** (implements `CustomDocument`)
...
**激活策略：**
- `onCustomEditor:minddoc.editor` — 打开 `.mind.md` 文件时激活（最小性能影响）
- 不使用 `onLanguage:markdown` 或 `workspaceContains`（避免不必要的早期激活）
- Explorer 右键菜单的"以 MindDoc 打开"通过 `menus.explorer/context` 声明式注册，不需要额外 activation event
...
`minddoc.toggleView` 使用图标区分当前视图状态（大纲图标 / 脑图图标），提供一键切换。
...
- MindDoc 文件通常单个打开，不太可能同时有大量 tab，内存影响可控
...
// Extension → Webview
type ExtToWebview =
  | { type: 'init'; tree: MindDocTree; settings: Settings; state: PersistedViewState | null }
...
- OPML/JSON 导出不经过 Webview — Extension host 直接调用 `@minddoc/core` 的 exportOpml/exportJson，因为不依赖 DOM
...
`@minddoc/core` 的 undo 模块仅提供：
- `invertOperation(op: Operation): Operation` — 计算操作的逆操作
- `applyOperation(tree: MindDocTree, op: PartialOperation): Operation` — 应用操作并返回完整 Operation（含 old 值，用于逆操作）
...
2. 读取 VSCode 设置（`minddoc.headingDepth`、`minddoc.defaultView` 等）
...
```
用户执行 minddoc.import.opml
  → Extension 弹出 showOpenDialog({ filters: { 'OPML': ['opml'] } })
  → 用户选择文件 → 读取 → @minddoc/core importOpml() 解析为 MindDocTree
  → Extension 弹出 showSaveDialog({ filters: { 'MindDoc': ['mind.md'] } })
...
**WebviewBridge** 提供与 Obsidian 版 `MindDocView` 相同的接口：

```typescript
class WebviewBridge {
  // Signals（组件通过这些信号读取状态）
  readonly tree: Signal<MindDocTree | null>;
...
```typescript
// key: "minddoc:viewState:{filePath}"
...
```
用户执行 minddoc.export.opml 命令
  → Extension 直接调用 @minddoc/core 的 exportOpml(tree)
...
### 命令
- [x] `minddoc.create` — 创建新 .mind.md 文件
- [x] `minddoc.openAs` — 以 MindDoc 打开当前 .md 文件（`vscode.openWith(uri, 'minddoc.editor')`）
- [x] `minddoc.toggleView` — 切换大纲/脑图
- [x] `minddoc.expandAll` — 展开全部
- [x] `minddoc.collapseAll` — 折叠全部
- [x] `minddoc.import.opml` — 导入 OPML（showOpenDialog → parse → showSaveDialog → writeFile → open）
- [x] `minddoc.import.freemind` — 导入 FreeMind（同上）
- [x] `minddoc.export.opml` — 导出 OPML
- [x] `minddoc.export.json` — 导出 JSON
- [x] `minddoc.export.png` — 导出 PNG
- [x] `minddoc.copyAIContext` — 复制为 AI 上下文

**命令 Enablement：** 所有需要活跃 MindDoc 编辑器的命令添加 enablement 条件：

```jsonc
"commands": [
  { "command": "minddoc.toggleView", "title": "MindDoc: Toggle View",
    "enablement": "activeCustomEditorId == 'minddoc.editor'" },
  { "command": "minddoc.expandAll", "title": "MindDoc: Expand All",
    "enablement": "activeCustomEditorId == 'minddoc.editor'" },
  { "command": "minddoc.export.png", "title": "MindDoc: Export as PNG",
    "enablement": "activeCustomEditorId == 'minddoc.editor'" }
  // ... 同理
]
```

`minddoc.create`、`minddoc.openAs`、`minddoc.import.*` 不需要 enablement（不依赖活跃编辑器）。

### 扩展配置（contributes.configuration）
- `minddoc.defaultView`: `"outline"` | `"mindmap"`（文件 frontmatter `default-view` 优先）
- `minddoc.headingDepth`: number (1-6, default 3)（文件 frontmatter `heading-depth` 优先）
- `minddoc.autoSaveDelay`: number (ms, default 300)
- `minddoc.outlineFontSize`: number (px, default 14)
- `minddoc.showNotePreview`: boolean (default true)
- `minddoc.mindmapDirection`: `"side"` | `"vertical"` | `"right"` | `"left"`
...
**全量 tree 传输：** 每次 operation 后通过 postMessage 广播完整 MindDocTree 给所有 Webview。对于大文件（1000+ 节点）的 structured clone 可能有数毫秒的性能代价。第一版接受此方案（简单、正确），后续可优化为增量传输（只发 operation + affected subtree 的 diff）。
